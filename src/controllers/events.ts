...
import { addUsersToEvent, findIDFromToken, insertEvent } from "../models/events";
    ...
    if(!token){
      ...
      try {
        ...
        if( accepted ) {
          ...
          const update = await pool.query(`UPDATE user_event SET accepted = TRUE WHERE event_id = $1 AND user_id = $2 RETURNING *`, [invitedEvent, userID.user_id])
          ...
          pool.query(`DELETE FROM user_event WHERE event_id = $1 AND user_id = $2`, [invitedEvent, userID.user_id])
