...
const encryptedEvent =
  'jH7L0tLCCrOluC5uKymDKHRRquK0GKtphWYODouf0KxeyXg98krGqrZ7AWAaOsC4eeM8';
...
it('sorts events into creator or invitee', async () => {
  const events = [
    { creator: true, event_id: '02001244-0da2-4876-952e-5f42c5e243fd' },
    { creator: false, event_id: '02001244-0da2-4876-952e-5f42c5e243fd' }
  ];
  const sortedEevents = eventSorter(events);
  expect(sortedEevents).toStrictEqual({
    invitedEvents: ['02001244-0da2-4876-952e-5f42c5e243fd'],
    myEvents: ['02001244-0da2-4876-952e-5f42c5e243fd']
  });
});
...
it('looks up events from event_id', async () => {
  ...
  expect(event).toStrictEqual({
    creator_id: 'a1edec9c-e63f-4129-96a0-c16b3ad75300',
    encrypted_event:
      'jH7L0tLCCrOluC5uKymDKHRRquK0GKtphWYODouf0KxeyXg98krGqrZ7AWAaOsC4eeM8',
    event_id: 'd70b1c24-218e-407a-bab2-f01f798d9862'
  });
});
...
it('adds relations of users and events', async () => {
  ...
  invitees.forEach(async (user) => {
    await insertUser(user, 'test2', 'secretKeyTest');
  });
  await addUsersToEvent(invitees, event_id, creator.user_id);
  const finalCount = (
    await pool.query('SELECT * from user_event WHERE event_id = $1', [event_id])
  ).rows.length;
